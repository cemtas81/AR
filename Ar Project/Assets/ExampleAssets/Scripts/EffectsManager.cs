...
public class EffectsManager : MonoBehaviour
{
    ...
    void TogglePlaneDetection()
    {
        ...
        togglePlaneDetectionButton.GetComponentInChildren<Text>().text = aRPlaneManager.enabled ? "Alan taramasını kapat" : "Alan taramasını aç";
    }

    void ToggleLights()
    {
        defaultLight.enabled = !defaultLight.enabled;
        toggleLightButton.GetComponentInChildren<Text>().text = defaultLight.enabled ? "Işıkları kapat" : "Işıkları aç";
    }
    ...
    void ToggleShadows()
    {
        if(defaultLight.enabled)
        {
            ...
            toggleShadowsButton.GetComponentInChildren<Text>().text = shadowValue == 0 ? "Gölgeleri aç" : "Gölgeleri kapat";
        }
    }
}
